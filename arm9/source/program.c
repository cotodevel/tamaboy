...
u12_t * program_load(uint32_t *size)
{
	uint32_t i;
	uint8_t buf[2];
	u12_t *program;
	...
	*size = rom_size/2;
	//printf("ROM size is %u * 12bits\n", *size);
	program = (u12_t *) TGDSARM9Malloc(*size * sizeof(u12_t));
	if (program == NULL) {
		printf("FATAL: Cannot allocate ROM memory !");
		return NULL;
	}
	for (i = 0; i < *size; i++) {
		buf[0] = rom[(i*2) + 0];
...
void program_save(char *path, u12_t *program, uint32_t size)
{
	...
	f = fopen(path, "w");
	if (f == NULL) {
		printf("FATAL: Cannot open ROM \"%s\" !");
		return;
	}
	...
	for (i = 0; i < size; i++) {
		...
		if (fwrite(buf, 1, 2, f) != 2) {
			printf("FATAL: Cannot write program from ROM !");
			fclose(f);
			return;
		}
	}
...
void program_get_data(u12_t *program, uint32_t size, char *path)
{
	...
	printf("Writing %u sprites to file %s ...", sprite_num, path);
...
void program_set_data(u12_t *program, uint32_t size, char *path)
{
	...
	printf("Reading %u sprites from file %s ...", img.height/10, path);

	sprite_num = generate_data_map(g_map, program, size);

	if (sprite_num != img.height/10)  {
		printf("FATAL: Invalid number of sprites (%u != %u) !", img.height/10, sprite_num);
		return;
	}
