...
void image_alloc(image_t *image)
{
	...
	image->row_pointers = (png_bytepp) TGDSARM9Calloc(sizeof(png_bytep) * image->height, 1);
	for (y = 0; y < image->height; y++) {
		image->row_pointers[y] = (png_byte*) TGDSARM9Calloc(image->stride, 1);
	}
}
...
void image_free(image_t *image)
{
	...
	for (y = 0; y < image->height; y++) {
		TGDSARM9Free(image->row_pointers[y]);
	}
	TGDSARM9Free(image->row_pointers);
}
